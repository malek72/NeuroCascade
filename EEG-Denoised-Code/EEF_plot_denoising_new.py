...
def extract_eeg_data(samples, channel_names):
    ...
    """
    eeg_data_list = [sample['eeg_data'].numpy() for sample in samples]
    ...
    with PdfPages(pdf_filename) as pdf:
        for content in report_content:
            ...
            if 'text' in content:
                text_fig = plt.figure()
                plt.axis('off')
                plt.text(0.5, 0.5, content['text'], ha='center', va='center', wrap=True)
...
def main():
    ...
    for preprocessed_file in selected_files:
        ...
        raw_samples_subject = [sample for sample in raw_dataset if sample['subject'] == subject_id]
        ...
        file_report_content.append({'text': intro_text, 'figure': plt.figure()})
        ...
        for ch in range(n_channels):
            f_raw, p_raw = welch(raw_data[ch], fs=sfreq, nperseg=sfreq*2)
            f_pre, p_pre = welch(preprocessed_data[ch], fs=sfreq, nperseg=sfreq*2)
            if freqs is None:
                freqs = f_raw
        ...
        f_raw, t_raw, Sxx_raw = spectrogram(raw_data[ch_idx, start_idx:end_idx], fs=sfreq)
        f_pre, t_pre, Sxx_pre = spectrogram(preprocessed_data[ch_idx, start_idx:end_idx], fs=sfreq)
        ...
        mean_raw = np.mean(raw_data[:, :end_idx], axis=0)
        mean_pre = np.mean(preprocessed_data[:, :end_idx], axis=0)

        fig = plt.figure(figsize=(12, 6))
        plt.plot(times[:end_idx], mean_raw, label='Raw')
        plt.plot(times[:end_idx], mean_pre, label='Preprocessed')
